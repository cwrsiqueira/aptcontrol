        ...
        <div class="row">
                ...
                <form action="{{ route('cc_client', ['id' => $client->id]) }}" method="get">
                    <div class="card-body">
                        <ul>
                            ...
                            @endforeach
                            <br>
                            <input class="mr-1" type="checkbox" name="entregas" value="1"
                                @if (!empty($_GET['entregas'])) checked @endif> Mostrar entregas Realizadas <br>
                        </ul>
                        {{-- <label for="">De:</label>
            ...
            <div class="col-md-3 m-3">
                <div class="card-tools">
                    <button class="btn btn-sm btn-secondary" onclick="javascript:history.go(-1);"
        ...
        <table class="table">
            ...
            <tbody>
                @foreach ($data as $item)
                    <tr
                        @if ($item->saldo < 0) style="color:green;font-weight:bold;" 
                    @elseif($item->saldo > 0 && $item->delivery_date < date('Y-m-d')) 
                    style="color:red;font-weight:bold;" 
                    @else 
                    style="color:#777;font-weight:bold;" @endif>
