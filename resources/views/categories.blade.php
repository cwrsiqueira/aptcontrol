    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg pt-3 px-4">
        ...
        <div>
            <div class="d-flex justify-content-between">
                ...
                <button @if (in_array('categories.create', $user_permissions) || Auth::user()->is_admin) @else disabled title="Solicitar Acesso" @endif
                    class="btn btn-secondary my-3" data-toggle="modal" data-target="#modal_addcategoria">Cadastrar
                    Categoria</button>
                        ...
                        <td>
                            @if (in_array('categories.update', $user_permissions) || Auth::user()->is_admin)
                                <a class="btn btn-sm btn-secondary"
                                    href="{{ route('categories.edit', ['category' => $item->id]) }}">Editar</a>
                            @else
                        ...
                        <td>
                            @if (in_array('categories.delete', $user_permissions) || Auth::user()->is_admin)
                                <form title="Excluir"
                                    ...
                                    onsubmit="return confirm('Confirma a exclusão da categoria?')">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="modal fade" id="modal_editcategoria">
            <div class="modal-dialog">
                <form class="form-horizontal" method="POST"
                    action="{{ route('categories.update', ['category' => $category->id ?? 0]) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <label for="name">Nome da Categoria:</label>
                            <input class="form-control @error('name') is-invalid @enderror" type="text" name="name"
                                placeholder="Nome da Categoria" id="edit_name" value="{{ $category['name'] ?? '' }}">
